...
namespace ObjectPooling
{
    public class ObjectPool<T>
    {
        ...
        public T GetItem()
        {
            ...
            foreach (ObjectPoolContainer<T> _ in _list)
            {
                _lastIndex++;
                
                if (_lastIndex > _list.Count - 1) _lastIndex = 0;
				
                if (_list[_lastIndex].IsUsed) continue;
        ...
        public void ReleaseItem(object item) => ReleaseItem((T) item);
        ...
        {
            for (int i = 0; i < capacity; i++) CreateContainer();
        }

        private ObjectPoolContainer<T> CreateContainer()
        {
            ObjectPoolContainer<T> container = new ObjectPoolContainer<T> {Item = _factoryFunc()};
